...
public String saveBillItems(List<ProductItem> items,
    ...
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, 3, 0); // Default to Retailer, no attender
}
...
public String saveBillItems(List<ProductItem> items,
    ...
    return saveBillItems(items, customerName, finalDiscount, payableAmount, grandTotal, uid, priceTotal, discountTotal, customerPhn, totalPaid, cashPaid, bankPaid, mode, type, balance, customerId, priceCategory, 0); // No attender
}
...
public String saveBillItems(List<ProductItem> items,
    ...
    try {
        ...
        // Generate bill number (e.g., 25-123 for year 2025, bill #123)
        java.util.Calendar cal = java.util.Calendar.getInstance();
        ...
        String sql = "INSERT INTO prod_bill (bill_display, total, extraDisc, payable, paid, uid, DATE, TIME, cusName, prodDisc, cusPhn, paymentMode, paymentType, balance, is_balance,currentBalance,customerId,price_category,attender_id) " +
             "VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW(), ?, ?, ?, ?, ?, ?, ?,?,?,?,?)";
...
/**
 * Returns the customer's available exchange points (customers.exchange_point).
 */
public double getCustomerExchangePoint(int customerId) throws Exception {
    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    try {
